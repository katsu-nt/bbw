...
export default function ExchangeContent() {
  ...
  useEffect(() => {
    dispatch(fetchExchangeCurrent({ base_currency: "USD", quote_currency: "VND" }));
    dispatch(fetchExchangeCurrent({ base_currency: "EUR", quote_currency: "VND" }));
  }, [dispatch]);
  ...
  const formatPrice = (rate) =>
    typeof rate === "number" ? rate.toLocaleString("vi-VN", { maximumFractionDigits: 2 }) : <ThreeDotsWave size="8px" color="#191919" />;
                ...
                usdVnd.delta_percent !== 0 && (
                  <span
                    className={`flex items-center ${
                      usdVnd.delta_percent > 0 ? "text-[#00B14F]" : "text-[#B51001]"
                    }`}
                  >
            ...
            <div className="text-2xl font-medium text-[#191919] flex gap-2 items-center">
              ...
              {typeof eurVnd?.delta_percent === "number" &&
                eurVnd.delta_percent !== 0 && (
                  <span
                    className={`flex items-center ${
                      eurVnd.delta_percent > 0 ? "text-[#00B14F]" : "text-[#B51001]"
                    }`}
                  >
                    <span className="text-sm inline-block -translate-y-2 transform">
                      {eurVnd.delta_percent > 0 ? "▲" : "▼"}
