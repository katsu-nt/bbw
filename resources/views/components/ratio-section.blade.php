...
<div class="bg-white rounded-xl border mt-6 balance-sheet-<?php echo $uniqueId; ?>">
            ...
            <tbody>
                ...
                <tr class="border-b last:border-none hover:bg-gray-50">
                    ...
                    <td class="py-6 px-6 text-center relative" style="min-width: 120px;">
                        <canvas class="ratio-trend-chart-<?php echo $index; ?>-<?php echo $uniqueId; ?>"
                            height="40"></canvas>
                    </td>
    ...
    <div id="chartPopup-<?php echo $uniqueId; ?>" class="bs-chart-popup" style="display: none;">
        <div class="chart-popup-content-bs">
            ...
            <canvas id="popupChart-<?php echo $uniqueId; ?>" style="margin-top: 20px;"></canvas>
        </div>
    </div>
</div>
        ...
        async function fetchRatioData(symbol, reportType) {
            try {
                const response = await fetch(
                    `http://103.97.127.42:8001/fundamental/ratio?symbol=${symbol}&report_type=${reportType}`
                );
                if (!response.ok) {
                ...
                console.error(`Lỗi khi fetch dữ liệu ${reportType}:`, error.message);
            ...
            fetchRatioData('<?php echo $symbol; ?>', type).then(data => {
                ...
                rowElements.forEach((row, rowIndex) => {
                    ...
                    const canvas = row.querySelector(
                        `canvas.ratio-trend-chart-${rowIndex}-${uniqueId}`);
                    if (canvas) {
        ...
        function initializeTableAndCharts() {
            fetchRatioData('<?php echo $symbol; ?>', 'quarterly').then(data => {
                ...
                const canvases = document.querySelectorAll(
                    `.balance-sheet-${uniqueId} [class^="ratio-trend-chart-"]`);
...
.ratio-trend-chart-0-<?php echo $uniqueId;
...
[class^="ratio-trend-chart-<?php echo $uniqueId; ?>"]:not(.ratio-trend-chart-0-<?php echo $uniqueId; ?>) {
    width: 100px;
    height: 40px;
    display: block;
    cursor: pointer;
...
?>:hover~.chart-popup-value {
    display: none;
}

[class^="ratio-trend-chart-"] {
