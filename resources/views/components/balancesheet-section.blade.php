...
$data = $balanceSheetFundamental['data'] ?? [];
...
$headersWithDates = collect($data)->mapWithKeys(function ($item) {
    if (strpos($item['key'], '_') !== false) {
        ...
        $quarter = (int)$quarter;
        if ($quarter >= 1 && $quarter <= 4 && preg_match('/^\d{4}$/', $year)) {
            try {
...
<div class="bg-white rounded-xl border mt-6 balance-sheet-<?php echo $uniqueId; ?>">
            ...
            <tbody>
                ...
                <tr class="border-b last:border-none hover:bg-gray-50">
                    ...
                    <td class="py-6 px-6 text-center relative" style="min-width: 120px;">
                        <canvas class="balanceSheetFundamental-<?php echo $index; ?>-<?php echo $uniqueId; ?>"
                            height="40"></canvas>
                    </td>
    ...
    <div id="chartPopup-<?php echo $uniqueId; ?>" class="bs-chart-popup" style="display: none;">
        <div class="chart-popup-content-bs">
            ...
            <canvas id="balanceSheetFundamentalPopup" style="margin-top: 20px;"></canvas>
        </div>
    </div>
</div>
...
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const popupChartCanvas = document.getElementById(`balanceSheetFundamentalPopup`);
        ...
        async function fetchRatioData(symbol, reportType) {
            try {
                const response = await fetch(
                    `http://103.97.127.42:8001/fundamental/balance-sheet?symbol=${symbol}&report_type=${reportType}`
                );
                if (!response.ok) {
        ...
        function processData(data, reportType) {
            ...
            const selectedKeys = Object.keys(headersWithDates).sort((a, b) =>
                headersWithDates[b].getTime() - headersWithDates[a].getTime()
            );
            const headers = selectedKeys.map(key => {
                if (key.includes('_')) {
                    ...
                    if (quarter >= 1 && quarter <= 4) {
                        ...
                        return `${year}`; // Hiển thị năm cho yearly
                    }
                }
                return key;
            });
        ...
        function updateTable(type) {
            fetchRatioData('<?php echo $symbol; ?>', type).then(data => {
                ...
                charts.forEach(chart => chart.destroy()); // Phá hủy chart cũ
                charts = []; // Reset mảng charts
                ...
                rowElements.forEach((row, rowIndex) => {
                    ...
                    const canvas = row.querySelector(
                        `canvas.balanceSheetFundamental-${rowIndex}-${uniqueId}`);
                    if (canvas) {
        ...
        function initializeTableAndCharts() {
            fetchRatioData('<?php echo $symbol; ?>', 'quarterly').then(data => {
                ...
                const canvases = document.querySelectorAll(
                    `.balance-sheet-${uniqueId} [class^="balanceSheetFundamental-"]`);
...
.balance-sheet-<?php echo $uniqueId;
...
.balanceSheetFundamental-0-<?php echo $uniqueId;
...
[class^="balanceSheetFundamental-<?php echo $uniqueId; ?>"]:not(.balanceSheetFundamental-0-<?php echo $uniqueId; ?>) {
    width: 100px;
    height: 40px;
    display: block;
    cursor: pointer;
...
#balanceSheetFundamentalPopup:hover~.chart-popup-value {
    display: none;
}

[class^="balanceSheetFundamental-"] {
